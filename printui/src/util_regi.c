...
static void ShowDialogRegiType001( LPREGICOMPTYPE001RES lpRegiRes, void *lpGWork )
{
	...
	gtk_window_set_transient_for( GTK_WINDOW(lpWork->regi_dialog), GTK_WINDOW(UI_DIALOG(g_main_window)->window) );
	gtk_main();
}
...
static void SetDefaultRegiType002( LPREGICOMPTYPE002RES lpRegiRes, void *lpGWork )
{
	...
	else if ( lpWork->uiType == UI_AUTOREGI_START_TYPE001 ){
                                        ...
                                        gpointer         user_data)
{
	...
	/* set return status */
	if ( (lpWk->uiType == UI_REGI_START_TYPE001) || (lpWk->uiType == UI_AUTOREGI_START_TYPE001) ){
		lpWk->status = US_ALIGIN_PRINT_HEAD;
	}
	else if ( lpWk->uiType == UI_REGI_START_TYPE002 ){
		...
		if ( current == REGI_TYPE002_BINVALUE_REARTRAY ){
			lpWk->status = US_ALIGIN_PRINT_HEAD_REARTRAY;
		}
		else if ( current == REGI_TYPE002_BINVALUE_CASETTE ){
			lpWk->status = US_ALIGIN_PRINT_HEAD_CASETTE;
		}
	}
	else {
		lpWk->status = -1;
                                        ...
                                        gpointer         user_data)
{
	...
	if ( (lpWk->uiType == UI_REGI_START_TYPE001) || (lpWk->uiType == UI_AUTOREGI_START_TYPE001) ){
		...
		gBinValueRegi = current;
		if ( current == REGI_TYPE002_BINVALUE_REARTRAY ){
			lpWk->status = US_PRINT_ALIGINMENT_VALUE_REARTRAY;
		}
		else{
			lpWk->status = US_PRINT_ALIGINMENT_VALUE_CASETTE;
		}
	}
	else {
		lpWk->status = -1;
                                        ...
                                        gpointer         user_data)
{
	...
	/* set return status */
	if ( lpWk->uiType == UI_BINSELECT_TYPE001 ){
		current = (short)GetActiveButtonIndex( window, regi_common_type002_radiobutton_name, REGI_TYPE002_BINVALUE_CASETTE );
		gBinValueRegi = current;
		if ( current == REGI_TYPE002_BINVALUE_REARTRAY ){
			lpWk->status = US_OK_REARTRAY;
		}
		else if ( current == REGI_TYPE002_BINVALUE_CASETTE ){
			lpWk->status = US_OK_CASETTE;
		}
	}
	else {
		lpWk->status = -1;
