...
{
	if( IsAvailableBorderless() )	/* Ver.3.00 */
		gtk_widget_show(LookupWidget(UI_DIALOG(main_window)->window,
			"borderless_vbox"));
}
...
void ShowUtilButtons(UIMainDialog* main_window)
{
	...
	if( util_flag & UF_CLEANING ) ShowUtilButton(main_window, "util_clean_button");

	if( util_flag & UF_DEEP_CLEANING ) ShowUtilButton(main_window, "util_refresh_button");

	if( util_flag & UF_PLATE_CLEANING ) ShowUtilButton(main_window, "util_plate_clean_button");

	if( util_flag & UF_ROLLER_CLEANING ){
		ShowUtilButton(main_window, "util_roller_clean_button");
	}

	if( util_flag & UF_NOZZLE_CHECK ) ShowUtilButton(main_window, "util_nozzle_check_button");

	if( util_flag & UF_HEAD_ALIGNMENT ) ShowUtilButton(main_window, "util_head_adjust_button");

	if( util_flag & UF_INK_RESET ) ShowUtilButton(main_window, "util_ink_reset_button");

	if( util_flag & UF_INK_WARNING ){
		ShowUtilButton(main_window, "util_ink_warning_button");
	}

	if( util_flag & UF_POWER_OFF ) ShowUtilButton(main_window, "util_power_off_button");

	if( util_flag & UF_AUTO_POWER ){
		InitCreate_SetConfigType001_AutoPower();
		ShowUtilButton(main_window, "util_auto_power_button");
	}

	if( util_flag & UF_CUSTOM ){
		InitCreate_SetconfigType001_CustomSettings();
		ShowUtilButton(main_window, "util_config_button");
	}

	if( util_flag & UF_QUIET ){
		InitCreate_SetConfigType001_QuietMode();
