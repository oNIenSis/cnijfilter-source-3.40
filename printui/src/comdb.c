...
gboolean IsAvailableBlackAdjustment()
{
	...
	for( i = 0 ; i < g_uidb.dbsize ; i++, lpdb++ )
	{
		if( lpdb->nObjectID == CNCL_BLACK_ADJUSTMENT
		 && lpdb->nValue == CND_BLACK_ADJUSTMENT && !lpdb->disable )
				return TRUE;
	}
	return FALSE;
}
...
{
	if( (GetCurrentnValue(CNCL_GRAYSCALE) == CND_BJGRAYSCALE_ON) &&
		(CNCL_ConfirmBlackAdjMedia( &g_uidb.nominfo, (void*)g_bjlibdir, g_uidb.lpdbTop, g_uidb.dbsize) != 0 ) ) return TRUE;
	return FALSE;
}
