...
static const short borderless_media_value[] =		//Ver.3.00:order changed.
{
	...
	CND_MEDIA_MATTE,
	CND_MEDIA_ADDRESS_POSTCARD,				/* Ver.3.20 */
	CND_MEDIA_INKJET_POST,
	CND_MEDIA_CONTENTS_GLOSSY,				/* Ver.3.20 */
	CND_MEDIA_CONTENTS_PROPHOTO,			/* Ver.3.20 */
	CND_MEDIA_POSTCARD,
...
static void InitBorderlessMedia(UIMediaBorderDialog* dialog)
{
	...
	for ( i=0; i<itemNum; i++ ){
		str = ValueToName( CNCL_MEDIATYPE, pBorderlessMediaValue[i] );
		dialog->media_list = g_list_append(dialog->media_list, str );
	}

	g_list_free(media_type_list_top);

	if ( pBorderlessMediaValue != NULL ) g_free( pBorderlessMediaValue );
Err:
	return;
}
